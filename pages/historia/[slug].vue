    ...
    <section class="section-spacing" :aria-label="chapter?.title">
      <div class="section-container">
        ...
        >
          ...
          {{ locale === 'es' ? 'Volver a la historia' : 'Back to the story' }}
        </NuxtLink>
        ...
        >
          <p class="text-sm text-ink-600 italic">
            {{ locale === 'es'
              ? 'Este capítulo aún no está disponible en inglés. Se muestra la versión en español.'
              : 'This chapter is not yet available in English. Showing the Spanish version.'
            }}
          </p>
        </div>
        ...
        <div v-if="!chapter" class="card-base text-center py-16">
          ...
          <p class="text-ink-500">{{ locale === 'es' ? 'Capítulo no encontrado.' : 'Chapter not found.' }}</p>
          <NuxtLink :to="localePath('/historia')" class="mt-4 inline-block text-sm text-ocean-600 hover:text-ocean-800 transition-colors">
            {{ locale === 'es' ? 'Ver todos los capítulos' : 'View all chapters' }}
          </NuxtLink>
        </div>
        ...
        <template v-else>
          ...
          <!-- Pagination -->
          <nav
            :aria-label="locale === 'es' ? 'Navegación entre capítulos' : 'Chapter navigation'"
